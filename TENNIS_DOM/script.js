...
let textLeft = Number(scoreLeft.textContent);
let textRigth = Number(scoreRigth.textContent);
...
let timer = 0; // по кнопке стар начинаем игру заново, чтобы удалить предыдущий таймер создаём переменную
...
function move () {
    ...
    if ((ballObject.posX <= racketLeftObject.posX + racketLeft.offsetWidth) &&
    ...
    if ((ballObject.posX + ball.offsetWidth >= racketRigthObject.posX) &&
